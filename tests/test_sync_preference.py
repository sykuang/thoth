...
class TestRepo:
    def test_upsert_creates_new(self, user_token: tuple[str, int]) -> None:
        ...
        row = user_sync_pref_repo.upsert(
            user_id=uid, hour=10, minute=0, tz="Asia/Taipei", enabled=True,
        )
    ...
    def test_upsert_updates_existing_preserves_last_run(
        ...
        _, uid = user_token
        user_sync_pref_repo.upsert(user_id=uid, hour=10, minute=0)
        ...
        # Update time
        row = user_sync_pref_repo.upsert(user_id=uid, hour=12, minute=0)
        assert row["hour"] == 12
        assert row["minute"] == 0
    ...
    def test_upsert_invalid_minute(self, user_token: tuple[str, int]) -> None:
        ...
        with pytest.raises(ValueError, match="minute"):
            user_sync_pref_repo.upsert(user_id=uid, hour=10, minute=60)

    def test_upsert_rejects_time_outside_fixed_slots(
        self, user_token: tuple[str, int],
    ) -> None:
        _, uid = user_token
    ...
    def test_upsert_rejects_non_taipei_timezone(
        self, user_token: tuple[str, int],
    ) -> None:
        _, uid = user_token
        with pytest.raises(ValueError, match="Asia/Taipei"):
            user_sync_pref_repo.upsert(user_id=uid, hour=10, minute=0, tz="UTC")

    def test_get_none_when_never_set(self, user_token: tuple[str, int]) -> None:
        _, uid = user_token
        assert user_sync_pref_repo.get(uid) is None

    def test_legacy_time_is_reported_disabled_and_not_scheduled(
        self, user_token: tuple[str, int],
    ) -> None:
        ...
        with get_conn() as conn:
            conn.execute(
                ...
                "VALUES (?, 9, 30, 'Asia/Taipei', 1, ?, ?)",
                (uid, now, now),
            )

        pref = user_sync_pref_repo.get(uid)
        assert pref is not None
        assert pref["enabled"] is False
        assert uid not in {row["user_id"] for row in user_sync_pref_repo.list_all_enabled()}
    ...
    def test_list_all_enabled_only(self) -> None:
        ...
        with get_conn() as conn:
            ...
            u2 = cur2.lastrowid
        assert u1 and u2
        user_sync_pref_repo.upsert(user_id=u1, hour=10, minute=0, enabled=True)
        user_sync_pref_repo.upsert(user_id=u2, hour=12, minute=0, enabled=False)
...
class TestRouter:
    ...
    def test_put_creates_and_get_returns_it(
        ...
        r = client.put(
            "/me/sync-preference",
            json={"hour": 18, "minute": 0, "tz": "Asia/Taipei", "enabled": True},
            headers=_auth(token),
        )
    ...
    def test_put_rejects_time_outside_fixed_slots(
        self, client: TestClient, user_token: tuple[str, int],
    ) -> None:
        token, _ = user_token
        r = client.put(
            "/me/sync-preference",
            json={"hour": 9, "minute": 0, "tz": "Asia/Taipei", "enabled": True},
            headers=_auth(token),
        )
        assert r.status_code == 422
        assert "10:00, 12:00, 18:00" in r.text
    ...
    def test_put_invalid_tz_returns_400(
        ...
        r = client.put(
            "/me/sync-preference",
            json={"hour": 10, "minute": 0, "tz": "Bogus/Foo"},
            headers=_auth(token),
        )
...
class TestSchedulerWiring:
    ...
    def test_reload_all_jobs_from_db(self, user_token: tuple[str, int]) -> None:
        ...
        user_sync_pref_repo.upsert(
            user_id=uid, hour=18, minute=0, enabled=True,
        )
        s = scheduler.get_scheduler()
        if not s.running:
            s.start()
